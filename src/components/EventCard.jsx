...
export default function EventCard({ event }) {
  return (
    <div className="group relative rounded-3xl overflow-hidden bg-[#070204] border border-red-950/80 hover:border-red-500/60 transition-all duration-300 hover:-translate-y-1.5 shadow-[0_0_20px_rgba(220,38,38,0.12)] hover:shadow-[0_0_35px_rgba(239,68,68,0.35)] flex flex-col justify-between">
      <div>
        <div className="relative aspect-[16/10] overflow-hidden bg-black">
          <Link to={`/events/${event.slug}`} className="block w-full h-full">
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[#070204] via-transparent to-black/40" />
          </Link>
        ...
        <div className="p-5 space-y-3">
          <Link to={`/events/${event.slug}`}>
            <h4 className="text-base font-bold text-white tracking-tight line-clamp-1 group-hover:text-red-400 transition-colors font-display">
              {event.title}
            </h4>
          </Link>
      ...
      <div className="p-5 pt-0">
        <Link
          ...
          className="w-full py-2.5 px-4 rounded-xl bg-red-950/40 hover:bg-gradient-to-r hover:from-red-600 hover:to-rose-600 text-xs font-bold text-red-300 hover:text-white border border-red-600/30 hover:border-transparent flex items-center justify-center gap-2 shadow-sm transition-all group/btn"
        >
